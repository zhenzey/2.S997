...
def visualize_pareto_front_interval(mechanisms,F,target_curve):
    """Draw Pareto Front in population and visualize mechanisms
    Parameters
    ----------
    mechanisms: numpy array [N,...]
    ...
    F_p = F_p[ind]
    interval = 50
    fig, axs = plt.subplots((X_p.shape[0]-1) // interval + 1, 3,figsize=(15,5*(X_p.shape[0]-1) // interval + 1))
    ...
    for i in trange(X_p.shape[0]):
        if i % interval == 0:
          C,x0,fixed_nodes,motor,target = from_1D_representation(X_p[i])
          draw_mechanism_on_ax(C,x0,fixed_nodes,motor,axs[i // interval,0])
          ...
          # Plot
          axs[i // interval,1].scatter(target_curve[:,0],target_curve[:,1],s=2)
          axs[i // interval,1].scatter(out_pc[:,0],out_pc[:,1],s=2)
          axs[i // interval,1].axis('equal')

          axs[i // interval,1].set_title('Chamfer Distance: %f'%(chamfer_distance(out_pc,target_curve)))
        
          axs[i // interval,2].scatter(F_p[:,1],F_p[:,0])
          axs[i // interval,2].set_xlabel('Material Use')
          axs[i // interval,2].set_ylabel('Chamfer Distance')
          axs[i // interval,2].scatter([F_p[i,1]],[F_p[i,0]],color="red")        
